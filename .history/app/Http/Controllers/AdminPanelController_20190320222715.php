...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filter_points!=null) {
            session::put('filter_points', $request->filter_points);
        }

        $search = $request->search ? '%'.$request->search.'%' : '%%';
dd(intval(session('filter_points')));
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ...
            ->when(session('filter_points_switch') == true && session('filter_points'), function($q) {
                $q->where('points', 'GREATER THAN', intval(session('filter_points')));
            })
