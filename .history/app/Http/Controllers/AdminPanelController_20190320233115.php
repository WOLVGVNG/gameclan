...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        session::put('filter_lang_pl', $request->filter_lang_pl ? 'pl' : null);
        session::put('filter_lang_en', $request->filter_lang_en ? 'en' : null);
        session::put('filter_lang_nl', $request->filter_lang_nl ? 'nl' : null);
        session::put('filter_lang_ge', $request->filter_lang_ge ? 'ge' : null);

        session::put('filter_deleted_switch', $request->filter_deleted_switch ?: false);
        if ($request->filter_deleted!=null) {
            session::put('filter_deleted', $request->filter_deleted);
        }
    ...
    // dd($request->filter_connected_accounts_steam);
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ...
            ->when(session('filter_deleted_switch') == true && session('filter_deleted'), function($q) {
                $q->where('email_verified_at', session('filter_deleted') == 'true' ? '!=' : '=', null);
            })
