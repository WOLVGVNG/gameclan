...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filter_active == 'active') {
            session::put('filter_active', 'active');
        }
        else if ($request->filter_active == 'banned') {
            session::put('filter_active', 'banned');
        }
        if($request->filter_active_switch && $request->filter_active == 'active') $db_active = 1;

        session::put('filter_notifiable_email_switch', $request->filter_notifiable_email_switch ?: false);
        if ($request->filter_active == true) {
            session::put('filter_notifiable_email', 'active');
        }
        else if ($request->filter_active == false) {
            session::put('filter_notifiable_email', 'banned');
        }
