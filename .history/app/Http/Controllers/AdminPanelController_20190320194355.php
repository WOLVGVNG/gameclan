...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        session::put('filter_banned', $request->filter_banned);
        session::put('filter_notifiable_email', $request->filter_notifiable_email);

        dd($request->filter_email);
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ->where('active', 'LIKE', session('filter_active')==true ?: '%%')
            ->where('banned', 'LIKE', session('filter_banned')==true ?: '%%')
            ->where('notifiable_email', 'LIKE', session('filter_notifiable_email')==true ?: '%%')
