...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        session::put('filter_active', $request->filter_active==true ? true : '%%');
        session::put('filter_banned', $request->filter_banned==true ? true : '%%');

        $search = $request->search ? '%'.$request->search.'%' : '%%';

        dd(session('filter_banned'));


        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ->where('active', $request->filter_active )
            ->orWhere('banned', session('filter_banned'))
