...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filter_notifiable_email!=null) {
            session::put('filter_notifiable_email', (string)$request->filter_notifiable_email);
        }
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ...
            ->where('notifiable_email', 'LIKE', session('filter_notifiable_email_switch')==true && session('filter_notifiable_email') ? session('filter_notifiable_email') : '%%')
