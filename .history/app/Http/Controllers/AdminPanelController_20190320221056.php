...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ->when(session('filter_active_switch') == true && session('filter_active') == 'active', function($q) {
                $q->where('active', 'LIKE', session('filter_active') == 'active' ? true : false);
            })
            ->when(session('filter_active_switch') == true && session('filter_active') == 'banned', function($q) {
                $q->where('banned', 'LIKE',  session('filter_active') == 'banned' ? true : false);
            })

            ->when(session('filter_active_switch') == true, function($q) {
                ->when( session('filter_active') == 'banned', function($q) {
            }
