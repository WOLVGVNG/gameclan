...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        session::put('filter_notifiable_email_switch', $request->filter_active_switch ?: false);
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ->where('active', 'LIKE', session('filter_active')=='active' && session('filter_active_switch')==true ? true : '%%')
            ->where('banned', 'LIKE', session('filter_active')=='banned' && session('filter_active_switch')==true ? true : '%%')
