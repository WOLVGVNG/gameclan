...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        session::put('filter_active_or_banned', $request->filter_active_or_banned ?: false);
        session::put('filter_active', $request->filter_active == 'active' ? 'active' : 'banned');

        session::put('filter_notifiable_email_or_not', $request->filter_active_or_banned ?: false);
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ->where('active', 'LIKE', session('filter_active')=='active' && session('filter_active_or_banned')==true ? true : '%%')
            ->where('banned', 'LIKE', session('filter_active')=='banned' && session('filter_active_or_banned')==true ? true : '%%')
