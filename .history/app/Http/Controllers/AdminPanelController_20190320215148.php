...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ->where('active', 'LIKE',
                session('filter_active_switch')==true
                && session('filter_active')=='active'
                ?
                    true
                    :
                    '%%')
            ->where('banned', 'LIKE',
                session('filter_active_switch')==true
                && session('filter_active')=='banned'
                ?
                    true
                    :
                    '%%')
            ->where('notifiable_email', 'LIKE',
                session('filter_notifiable_email_switch')==true
                && session('filter_notifiable_email')
                ?
                    (session('filter_notifiable_email')=='true'
                    ?
                        true
                        :
                        false)
                    :
                    '%%')
            ->where('email_verified_at', 'LIKE',
                session('filter_verified_email_switch')==true
                && session('filter_verified_email')
                ?
                    (session('filter_verified_email')=='true'
                    ?
                        '!='
                        :
                        '=')
