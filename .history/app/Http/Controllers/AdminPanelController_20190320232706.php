    ...
    // dd($request->filter_connected_accounts_steam);
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ...
            ->when(session('filter_lang_switch') == true, function($q) {
                $q->when(session('filter_lang_poland') == 'pl', function($w) {
                    $w->where('lang', '=', 'pl');
                });
                $q->when(session('filter_lang_england') == 'en', function($w) {
                    $w->where('lang', '=', 'en');
                });
                $q->when(session('filter_lang_germany') == 'ge', function($w) {
                    $w->where('lang', '=', 'ge');
                });
                $q->when(session('filter_lang_netherland') == 'nl', function($w) {
                    $w->where('lang', '=', 'nl');
                });
            })
