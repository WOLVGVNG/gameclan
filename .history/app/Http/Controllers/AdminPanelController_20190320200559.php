...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ->where('active', 'LIKE', session('filter_active')=='active' && session('filter_active_or_banned')==true ?: '%%')
            ->where('banned', 'LIKE', session('filter_active')=='banned' && session('filter_active_or_banned')==true ?: '%%')
