...
class AdminPanelController extends Controller
{
    ...
    // dd($request->filter_connected_accounts_steam);
        ...
        $users = User::orderBy($request->sort_type, $request->sort_order)
            ->when($request->filter_active_switch == true, function($q) use($request) {
            ...
            ->when($request->filter_notifiable_email_switch == true && $request->filter_notifiable_email, function($q) use($request) {
                $q->where('notifiable_email', 'LIKE', ($request->filter_notifiable_email == 'true' ? true : false));
            })
            ->when($request->filter_verified_email_switch == true && $request->filter_verified_email, function($q) use($request) {
                $q->where('email_verified_at', $request->filter_verified_email == 'true' ? '!=' : '=', null);
            })
            ->when($request->filter_points_switch == true, function($q) use($request) {
