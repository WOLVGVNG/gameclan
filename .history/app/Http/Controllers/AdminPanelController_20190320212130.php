...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ...
            ->where('email_verified_at', 'LIKE', session('filter_email_verified_at_switch')==true && session('filter_email_verified_at') ? (session('filter_email_verified_at')=='true' ? true : false) : '%%')
