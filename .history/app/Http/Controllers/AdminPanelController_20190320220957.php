...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ->when(session('filter_active_switch') == true && session('filter_active') == 'active', function($g) {
                $g->where('active', 'LIKE', session('filter_active') == 'active' ? true : false);
            })
            ->when(session('filter_active_switch') == true && session('filter_active') == 'banned', function($g) {
