...
//dd(session('filter_email_verified'));
        ...
        $users = User::orderBy(session('sort_type'), session('sort_order'))
            ...
            ->where('email_verified_at', 'LIKE', session('filter_verified_email_switch')==true && session('filter_verified_email') ? (session('filter_verified_email')=='true' ? true : null) : '%%')
